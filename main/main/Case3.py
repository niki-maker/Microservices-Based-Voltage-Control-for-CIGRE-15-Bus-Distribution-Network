...
def parse_and_export():
    try:
        ...
        # ✅ Inject 13 MVAR capacitor at node N3 after initial solve
        try:
            node_n8 = system.get_node_by_uuid("N8")
            injected_q_mvar = 3.8
            node_n8.reactive_power = injected_q_mvar / base_apparent_power
            logging.info(f"🔌 Injected {injected_q_mvar} MVAR capacitor at N3")
        except Exception as e:
            logging.error(f"⚠️ Failed to inject capacitor at N3: {e}")
        results_pf = nv_powerflow.solve(system)[0]

        # Display voltages after the first power flow analysis
        print("\nVoltages after First Powerflow Analysis:")
        for node in results_pf.nodes:
            voltage_magnitude = abs(node.voltage_pu)
            voltage_angle = np.angle(node.voltage_pu, deg=True)
            print(f"{node.topology_node.name} = {voltage_magnitude:.3f} ∠ {voltage_angle:.2f}°")
