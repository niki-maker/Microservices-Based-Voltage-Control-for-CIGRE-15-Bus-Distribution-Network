...
def priority_score_exporter(system, path="/shared_volume/priority_score.xlsx"):
    try:
        ...
        for node in system.nodes:
            ...
            # Recompute scores inline
            r_imp = getattr(node, "r_imp", 1.0)
            r_sens = getattr(node, "r_sens", 1.0)
            r_elec = 0.6 * r_imp + 0.4 * r_sens

            load_score = getattr(node, "load_score", 0.0)
            ease_score = getattr(node, "ease_score", 0.0)
...
def optimize_powerflow(base_apparent_power=25):
    try:
        ...
        # ---------------- Ease-of-Influence Score ----------------
        logging.info("🧲 Computing ease-of-influence scores...")
        ...
        # Compute hop counts from slack
        hop_counts = nx.single_source_shortest_path_length(G, slack_uuid)
        all_hops = [hop_counts.get(n.uuid, float("inf")) for n in system.nodes]
        ...
        for node in system.nodes:
            ...
            normalized_hops = (hops - min_hop) / (max_hop - min_hop) if max_hop > min_hop else 1.0

            device_flag = 1.0 if (
                name in capacitor_reactive_power or
                name in shunt_reactor_reactive_power or
                any(name in pair for pair in tie_switches)
            ) else 0.0

            ease_score = 0.7 * (1 - normalized_hops) + 0.3 * device_flag
            logging.info(f"🪜 Bus {name} | Hops = {hops} | Device = {device_flag} | Ease Score = {ease_score:.4f}")

        # ---------------- Priority Score Calculation ----------------
        logging.info("🎯 Computing final priority scores...")

        # Step 1: Compute apparent power magnitude for load criticality
        load_mags = {
            node.uuid: math.sqrt(node.power.real**2 + node.power.imag**2)
            for node in system.nodes
        }
        max_load = max(load_mags.values())
        min_load = min(load_mags.values())

        load_scores = {
            uuid: (mag - min_load) / (max_load - min_load) if max_load > min_load else 0.0
            for uuid, mag in load_mags.items()
        }

        # Step 2: Reuse previously computed scores
        # - r_elec from hybrid remoteness
        # - ease_score from hop + device presence
        ...
        for node in system.nodes:
            ...
            # ✅ Store scores on node for export
            node.r_imp = r_imp
            node.r_sens = r_sens
            node.load_score = load_score
            node.ease_score = ease_score

            logging.info(f"🏁 Bus {name} | R_elec = {r_elec:.4f} | Load = {load_score:.4f} | Ease = {ease_score:.4f} | Priority Score = {priority_score:.4f}")
